...
$this->title = Yii::t('cherryii/guestbook', 'View post');
...
<dl class="dl-horizontal guestbook-view">
    ...
    <dt><?= Yii::t('cherryii', 'Text') ?></dt>
    <dd><?= nl2br($model->text) ?></dd>
</dl>

<hr>
<h2><small><?= Yii::t('cherryii/guestbook', 'Answer') ?></small></h2>
...
<?= Html::beginForm() ?>
    ...
    <div class="checkbox">
        <label>
            <input type="checkbox" name="mailUser" value="1" checked> <?= Yii::t('cherryii/guestbook', 'Notify user about answer') ?>
        </label>
    </div>
    <?php endif; ?>
    <?= Html::submitButton(Yii::t('cherryii', 'Save'), ['class' => 'btn btn-success send-answer']) ?>
<?= Html::endForm() ?>