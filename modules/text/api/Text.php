<?php
namespace yii\cherryii\modules\text\api;

use Yii;
use yii\cherryii\components\API;
use yii\cherryii\helpers\Data;
use yii\helpers\Url;
use yii\cherryii\modules\text\models\Text as TextModel;
use yii\helpers\Html;

/**
 * Text module API
 * @package yii\cherryii\modules\text\api
 *
...
class Text extends API
{
    ...
    private function notFound($id_slug)
    {
        ...
        if(!Yii::$app->user->isGuest && preg_match(TextModel::$SLUG_PATTERN, $id_slug)){
            $text = Html::a(Yii::t('cherryii/text/api', 'Create text'), ['/admin/text/a/create', 'slug' => $id_slug], ['target' => '_blank']);
        }
