<?php
namespace yii\cherryii\modules\subscribe\controllers;
...
class AController extends Controller
{
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            else
            {
                if($model->validate() && $this->send($model)){
                    $this->flash('success', Yii::t('cherryii/subscribe', 'Subscribe successfully created and sent'));
    ...
    public function actionDelete($id)
    {
        ...
        return $this->formatResponse(Yii::t('cherryii/subscribe', 'Subscriber deleted'));
    }
    ...
    private function send($model)
    {
        ...
        foreach(Subscriber::find()->all() as $subscriber){
			$unsubscribeLink = '<br><a href="' . Url::to(['/admin/'.$this->module->id.'/send/unsubscribe', 'email' => $subscriber->email], true) . '" target="_blank">'.Yii::t('cherryii/subscribe', 'Unsubscribe').'</a>';
