<?php
namespace yii\cherryii\modules\catalog\controllers;
...
use yii\cherryii\behaviors\SortableDateController;
...
class ItemsController extends Controller
{
    ...
    public function actionCreate($id)
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            if(Yii::$app->request->isAjax){
                ...
                if ($model->save()) {
                    $this->flash('success', Yii::t('cherryii/catalog', 'Item created'));
    ...
    public function actionEdit($id)
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            if(Yii::$app->request->isAjax){
                ...
                if ($model->save()) {
                    $this->flash('success', Yii::t('cherryii/catalog', 'Item updated'));
                    return $this->redirect(['/admin/'.$this->module->id.'/items/edit', 'id' => $model->primaryKey]);
                } else {
    ...
    public function actionClearImage($id)
    {
        ...
        elseif($model->image){
            ...
            if($model->update()){
                ...
                $this->flash('success', Yii::t('cherryii', 'Image cleared'));
            } else {
                $this->flash('error', Yii::t('cherryii', 'Update error. {0}', $model->formatErrors()));
            }
        }
    ...
    public function actionDelete($id)
    {
        ...
        return $this->formatResponse(Yii::t('cherryii/catalog', 'Item deleted'));
    }
    ...
    private function generateForm($fields, $data = null)
    {
        ...
        foreach($fields as $field)
        {
            ...
            elseif ($field->type === 'select') {
                $options = ['' => Yii::t('cherryii/catalog', 'Select')];
