...
use yii\cherryii\models\CopyModuleForm;
...
class ModulesController extends \yii\cherryii\components\Controller
{
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            if(Yii::$app->request->isAjax){
                ...
                if($model->save()){
                    $this->flash('success', Yii::t('cherryii', 'Module created'));
    ...
    public function actionEdit($id)
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            if(Yii::$app->request->isAjax){
                ...
                if($model->save()){
                    $this->flash('success', Yii::t('cherryii', 'Module updated'));
                }
                else{
    ...
    public function actionSettings($id)
    {
        ...
        if (Yii::$app->request->post('Settings')) {
            ...
            if($model->save()){
                $this->flash('success', Yii::t('cherryii', 'Module settings updated'));
            }
            else{
                $this->flash('error', Yii::t('cherryii', Yii::t('cherryii', 'Update error. {0}', $model->formatErrors())));
            }
            return $this->refresh();
        }
        else {
    ...
    public function actionRestoreSettings($id)
    {
        if(($model = Module::findOne($id))){
            $model->settings = '';
            $model->save();
            $this->flash('success', Yii::t('cherryii', 'Module default settings was restored'));
        } else {
            $this->flash('error', Yii::t('cherryii', 'Not found'));
        }
        return $this->back();
    ...
    public function actionCopy($id)
    {
        ...
        if ($formModel->load(Yii::$app->request->post())){
            if(Yii::$app->request->isAjax){
                ...
                $moduleFileContent = str_replace('@cherryii', '@app', $moduleFileContent);
                ...
                foreach(new \RecursiveIteratorIterator(new \RecursiveDirectoryIterator($newModuleFolder)) as $file => $object){
                    if(!$object->isDir()){
                        ...
                        $fileContent = str_replace("Yii::t('cherryii/".$module->name, "Yii::t('cherryii/".$formModel->name, $fileContent);
                ...
                foreach(glob($newModuleFolder . DIRECTORY_SEPARATOR . 'models' . DIRECTORY_SEPARATOR . '*.php') as $modelFile){
                    ...
                    $newTableName = str_replace('cherryii', 'app', $newTableName);
    ...
    public function actionDelete($id)
    {
        ...
        return $this->formatResponse(Yii::t('cherryii', 'Module deleted'));
    }
